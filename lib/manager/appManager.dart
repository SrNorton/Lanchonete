...
import 'package:lanchonete_android_project/components/cardClienteChoice.dart';
...
import 'package:lanchonete_android_project/models/notices.dart';
...
Future<void> deleteAllOrders(BuildContext context) async {
  final CollectionReference orderCollection = await FirebaseFirestore.instance.collection('order');

  try {
    final QuerySnapshot querySnapshot = await orderCollection.get();

    for (QueryDocumentSnapshot document in querySnapshot.docs) {
      await document.reference.delete();
    }

   StatusMessage(statusSucces: true).showMySnackBar(context: context, msg: 'Você deletou os pedidos');

    print('pedidos excluidos');
  } catch (e) {
    print('erro ao deletar pedidos $e');
  }
}
